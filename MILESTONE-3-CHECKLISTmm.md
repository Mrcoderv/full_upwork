...
**Verification method:** Static code review of backend (Express/Mongoose, ESM) + frontend (Vue 3/Vuetify).  
**Table of Contents**  
**Etapp 1 — Items 1–25**  
1. [Profiler med olika behörigheter (Elev, Lärare, Admin, Systemadmin, SYV, Specialpedagog, Praktiksamordnare) + personanpassning](#anchor-1 "#anchor-1")  
2. [Sökruta — datum, elev, lärare, kurs; räcker med tre tecken](#anchor-2 "#anchor-2")  
3. [Elevprofil — Flik 1 Allmänt (kontaktuppgifter, kommentarer, behov av stöd, avvikelse)](#anchor-3 "#anchor-3")  
4. [Elevprofil — Flik 2 Studieplan (kurser/kurspaket, lärare, status, reviderad)](#anchor-4 "#anchor-4")  
5. [Elevprofil — Flik 3 APL (bara för kurspaketselever, inte personal)](#anchor-5 "#anchor-5")  
6. [Elevprofil — Flik 4 Behörigheter (se/ändra behörigheter)](#anchor-6 "#anchor-6")  
7. [Elevprofil — Flik 5 Dokument (handlingsplan, CV, APL-kontrakt osv.)](#anchor-7 "#anchor-7")  
8. [Elevprofil — Flik 6 Kursarkiv (slutprov, delprov etc.)](#anchor-8 "#anchor-8")  
9. [Kurser — samlad sida + "lägg till" kursplacering (datahämtning Alvis)](#anchor-9 "#anchor-9")  
10. [Kursplacering — Kurs (startdatum, 5/10/20 v, personuppgifter, slutprov auto, stöd, plats/distans)](#anchor-10 "#anchor-10")  
11. [Kursplacering — Kurspaket (studietakt 100/50/25 %, fullt/reviderat paket, auto datum, auto APL, tidigare praktik)](#anchor-11 "#anchor-11")  
12. [Avbrott (bekräftelse till lärare, bort från slutprov/APL, markering/inaktiv lista)](#anchor-12 "#anchor-12")  
13. [Revidering (planera om studieplan, auto-uppdatera slutprov/APL, bekräftelse till lärare+elev)](#anchor-13 "#anchor-13")  
14. [Inaktiva elever / återkommande elev (auto-fyll uppgifter, lista lästa kurser)](#anchor-14 "#anchor-14")  
15. [APL-listan (flikar, 6 färger + filter, auto-röd)](#anchor-15 "#anchor-15")  
16. [Betygssättning (auto-påminnelse, sida per lärare, A–F, motivering, kommentarer, obligatoriskt)](#anchor-16 "#anchor-16")  
17. [Nationella prov (Engelska/Svenska/Matematik poäng) + betygsskala ändras årligen (systemadmin)](#anchor-17 "#anchor-17")  
18. [Lås/upplås betyg (lärare låser, admin meddelas, systemadmin/admin låser upp)](#anchor-18 "#anchor-18")  
19. [Signering av betygskataloger (Scrive — ladda upp PDF:er en och en, skicka för digital signering)](#anchor-19 "#anchor-19")  
20. [Handlingsplan (F → påminnelse kvar tills ifylld, frågeformulär, PDF, systemadmin ändrar frågorna)](#anchor-20 "#anchor-20")  
21. [Prövningar (bokning, intresselista, notis 3–4 v före månadsslut, Godkänn/Flytta/Neka, material, betalning, auto till slutprov)](#anchor-21 "#anchor-21")  
22. [Slutprovssida / kalendern (månadsvis, lärarens färg, lista elever, närvaro, anpassning, lärare ändrar datum, admin väljer plats/rum)](#anchor-22 "#anchor-22")  
23. [SYV (se elev, boka möten i kalender, info i elevkort, revidera studieplaner)](#anchor-23 "#anchor-23")  
24. [Specialpedagog (se elev, boka möten, info i elevkort, anpassningar för slutprov)](#anchor-24 "#anchor-24")  
25. [Ekonomi och rapporter (intäkter per kommun/kurs, månadsprognos, elevstatistik, avbrott, F-betyg, betygskurva)](#anchor-25 "#anchor-25")  
**Etapp 2 — Items 26–43**  
1. [Sollentuna auto-email — "Lärteamet" (Learning Team) after admission](#anchor-26 "#anchor-26")  
2. [Internal messaging/chat (staff–staff, school–student, email copies, SMS-style exploration)](#anchor-27 "#anchor-27")  
3. [Chatbot (answers common student questions from existing documentation)](#anchor-28 "#anchor-28")  
4. [Learning platform (student sees study plan + course cards; lessons, instructions, assignments; teacher feedback; per-course progress)](#anchor-29 "#anchor-29")  
5. [Course templates (admin-created, per-course; select teachers can create; 5 modules × 2 sections; Module 3 = partial exams, Module 5 = case study; duplicated into course cards)](#anchor-30 "#anchor-30")  
6. [Course cards (created at admission, follow study plan; same course+dates share a card with responsible teacher; shows course name, start, end, study period, weeks; auto-generate from templates + enrollment dates)](#anchor-31 "#anchor-31")  
7. [Inactive-student automation (no login within first 5 days → mandatory withdrawal; 14 days inactivity → warning email; detect via last login, lesson-date adherence, assignment-schedule adherence; admin chooses immediate withdraw [auto-email + teacher notification, cascade to package courses] or warning email with named future withdrawal date from saved email templates)](#anchor-32 "#anchor-32")  
8. [Course card — overview/first page (activity feed; staff-only notice board [students read, cannot post] for welcome/exam info/announcements; which assignments submitted; which section each student is on)](#anchor-33 "#anchor-33")  
9. [Course card — assignments (return for correction/revision; inline comments on the assignment or assignment-tied comments; student–teacher discussion thread per assignment)](#anchor-34 "#anchor-34")  
10. [Course card — date planning (teachers manually schedule section dates ~2 weeks before start; desired: reusable per-course teacher "parameters" for 5/10/20-week lengths; auto-generate schedule from teacher's saved parameters + start date)](#anchor-35 "#anchor-35")  
11. [Course card — content (admins and granted teachers view/edit all components; components can be hidden from student view)](#anchor-36 "#anchor-36")  
12. [Course card — reports (per-component completion: green ✓ completed / grey ✗ not completed; feeds withdrawal/warning automation; click student for detail; desired: scheduled-date column to distinguish "not done" from "not yet scheduled")](#anchor-37 "#anchor-37")  
13. [Course card — participants (view/add/remove students and staff on the card; automatic removal on withdrawal or staff departure; desired: last-access column)](#anchor-38 "#anchor-38")  
14. [APL — logbook (APL-only students get personalized "kits" instead of standard course cards, sent at internship start, visible on student home page)](#anchor-39 "#anchor-39")  
15. [APL — activity (color codes indicating whether a student has fallen behind schedule)](#anchor-40 "#anchor-40")  
16. [Certificates — study certificate (one-click generation from student profile, Alvis-like)](#anchor-41 "#anchor-41")  
17. [Certificates — diploma (course-package students; notification after package end date once complete; one click generates only after verifying every course approved AND APL approved; signed diploma generated and sent to student)](#anchor-42 "#anchor-42")  
18. [Question bank & exam generation (a bank of exam questions exists; the platform can generate new exams by drawing from it)](#anchor-43 "#anchor-43")  
...
9. **Lås betyg (#18): ✅ Implemented** — lock now teacher+admin/systemadmin per spec (ALLOWED_GRADING_ROLES, gradeRoutes.js:31), and every lock creates a grade_locked admin notification (gradeRoutes.js:595-606, notificationTypes.js GRADE_LOCKED). Unlock stays admin/systemadmin-only (spec).  
...
**Gap Summary — Etapp 1 (All Partial Items, Consolidated)**  
| | | | |  
|-|-|-|-|  
